function [MMR_opt_chain, AllSolution, r_uav_chain] = Proposed_Alg(channel,chan)

rng('default'); 
rng(1); 

[r_uav_current, phi_current, tau_current, p_current, varphi_current, slack_1, slack_2, slack_3] = initilize(channel,chan);
% load('phi_current.mat');

MMR_opt = 0;
MMR_opt_currrent = 0;
MMR_opt_chain = [];
r_uav_chain = [r_uav_current];
...
while(1)
    if (status_1 == 1)
        ...
        [AllSolution, Status] = Get_Solution_1(channel, Init, r_uav_current, phi_current, p_current, varphi_current, tau_current, slack_1);
        %% Update
    ...
    if (status_2 == 1 && dem1 >= 1)  
        if Init_1 == 1
           fprintf('Inilizing- Iteration %d -th \n', iter);
        else
           fprintf('Updating_2- Iteration %d -th \n', iter);
        end
        [AllSolution, Status] = Get_Solution_2 (channel, Init_1, r_uav_current, phi_current, p_current, varphi_current, tau_current, slack_2);
        %% Update
        if (~isempty(findstr(Status,'Successfully')))
            MMR_opt = AllSolution{1};
    ...
    if (status_3 == 1 && dem2 >= 1) 
        if Init_2 == 1
           fprintf('Inilizing- Iteration %d -th \n', iter);
        else
           fprintf('Updating_3- Iteration %d -th \n', iter);
        end
        [AllSolution, Status] = Get_Solution_3_1 (channel, chan, Init_2, r_uav_current, phi_current, p_current, varphi_current, tau_current, slack_3);
        %% Update
        if (~isempty(findstr(Status,'Successfully')))
            MMR_opt = AllSolution{1};
    ...
    if(MMR_opt - MMR_opt_currrent <= 1e-3  && iter >= 30)
        break;
    end
